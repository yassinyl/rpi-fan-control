...
# The script is run from the cloned directory, so we act on it directly.
sudo chown -R pi:pi .
chmod +x pwm-fan-control.py install.sh

# Step 3: Setup the systemd service
echo "[3/4] Setting up the systemd service..."
# Copy the service file from the current directory to the systemd folder
