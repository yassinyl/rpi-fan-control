...
class FanController:
    ...
    def __init__(self):
        ...
        # Wait for pigpio daemon (important on Trixie boot)
        self.pi = None
        ...
        for _ in range(10):
            ...
            print("[INFO] Waiting for pigpio daemon...")
            time.sleep(1)
    ...
    def load_config_values(self):
        ...
        self.startup_duration = self.config.get(
            "startup_duration",
            2
        )
    ...
    def get_speed_from_temp(self, temp):
        ...
        if temp <= self.temp_low:
            ...
            speed = self.rpm_min + (
                scale * (self.rpm_max - self.rpm_min)
            )
    ...
    def run(self):
        ...
        while True:
            ...
            if cpu >= self.cpu_threshold:
                ...
                self.force_mode
                and time.time() - self.force_start
                ...
                log(
                    f"CPU usage dropped to "
                    f"{cpu}% → Resuming auto mode"
                )
            ...
            if not self.force_mode:
                ...
                if (
                    ...
                    if (
                        abs(speed - self.last_speed) >= 1
                        or speed in (0, 100)
                    ):

                        self.set_fan_speed(
                            speed,
                            reason=f"[Temp {temp:.1f}°C]"
                        )
